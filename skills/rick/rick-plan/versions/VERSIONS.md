...
- v6 | 2026-06-24 | pre-loop snapshot
- v6 | 2026-06-24 | targeted-fix | scope: PICK-FOLDER.md (not SKILL.md)
  - Round 1: gated priority 1 of "Pick the folder" on `$ARGUMENTS` leading issue number. If an issue number was passed, priority 1 fires only if the current branch name contains those digits. Mirrors the rick-intel folder-resolution fix (PR #14). Failure mode prevented: `/rick-plan 50 do X` run on `feature/template-mgmt` was writing the new plan into the unrelated feature's folder. The "I'm already on the right branch" convenience case is preserved when the branch and issue actually match (e.g. `feat/50-rotate-share-token` + `/rick-plan 50`).
  - Round 2: stopped early (single-purpose fix; no further round needed)
  - Round 3: stopped early
  - Restore pre when: a workflow comes up where the user wants `/rick-plan <n> ...` to forcibly adopt the current branch even when the branch name doesn't reference the issue (e.g. an arbitrary branch is intentionally being repurposed). v5's unconditional priority 1 supported that without ceremony.
  - Restore post when: another round of /rick-plan invocations on unrelated branches confirms the gate stops mis-routed plans without ever blocking a legitimate "branch matches the issue" run.
