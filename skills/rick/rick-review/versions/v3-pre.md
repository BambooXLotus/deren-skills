...
| Rick Prime     | `/tmp/rr-diff-broad.txt`      | inherit   | `grep -qE '^\+(import\|export).*from .*\.\./' /tmp/rr-diff-broad.txt` OR `git diff $BASE_REF --diff-filter=A --name-only \| grep -qE '^src/[^/]+/[^/]+$'` (new top-level `src/` dir) |
...
**2. Hedge scan.** If the row contains "could silently," "if a future edit," "in case someone changes," "after a refactor," "for symmetry," "for parity," "for completeness," "hypothetical" — **demote one tier**. P3 demoted from this gate is **killed**.
