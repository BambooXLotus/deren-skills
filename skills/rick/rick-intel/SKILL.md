...
After writing the file, post an HTML comment to the ADO work item. ADO renders comments as HTML; plain text is unreadable.
...
Idempotency: fetch existing comments. If one contains the disclaimer `This was generated by AI during research.`, update it via the ADO REST API instead of posting new. If update isn't possible from the CLI, post new and accept the duplicate (warn the user).
